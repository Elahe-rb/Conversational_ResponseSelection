...
import preprocess_data, train, smn, dual_encoder, smn_smn, evaluate, smn_paper
...
train_rows, valid_rows, test_rows, vocab, train_uids_rows, valid_uids_rows, test_uids_rows = preprocess_data.load_Data(args)
...
for epoch in range(num_epochs):
    ...
    model, train_losses = train.train(model, loss_fn, optimizer, train_rows, batch_size, epoch, num_epochs, vocab, device, train_uids_rows, args)

    with torch.no_grad():
        val_R1, val_R2, val_R5, acc = evaluate.evaluate(model, valid_rows, evaluate_batch_size, epoch, num_epochs, vocab, max_conv_utt_num, max_utterance_length, device, valid_uids_rows, args)
...
# Run on test data.
test_R1, test_R2, test_R5, test_acc = evaluate.evaluate(model, test_rows, evaluate_batch_size, 0, num_epochs, vocab, max_conv_utt_num, max_utterance_length, device, test_uids_rows, args)
