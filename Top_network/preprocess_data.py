...
def numberize_rnn(data, vocab, max_utt_num, max_utt_length, device, cluster_ids, vocabs, is_mini_net):
    max_len = max_utt_num * max_utt_length
    cs = []
    rs = []
    ys = []
...
def process_data(rows, batch, batch_size, vocab, args, device, cluster_ids, vocabs, is_mini_net):

    batched_rows = get_batch(rows, batch, batch_size)

    if args.isHier:
        cs, rs, ys, clusters = numberize_smn(batched_rows, vocab, args.maxUttNum, args.maxUttLen, device)
    else:
        cs, rs, ys, clusters = numberize_rnn(batched_rows, vocab, args.maxUttNum, args.maxUttLen, device, cluster_ids, vocabs, is_mini_net)
