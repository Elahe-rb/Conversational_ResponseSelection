...
class SMN(nn.Module):
    ...
    def init_weights(self):
        ...
        glove_embeddings = preprocess_data.load_glove_embeddings(self.vocab, self.emd_dir)
    ...
    def forward(self, utterance, response, scores):
        '''
        ...
        res = (logits.mm(self.M1).to(device)) + (scores.mm(self.M2).to(device))

        results = torch.sigmoid(res)  # dim: batchsize * 1

        return results     #dim: batchsize*1
